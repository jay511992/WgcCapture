    ...
    const int exit_code = [&]() -> int {
        ...
        if (!capture_test::ValidateGw2Args(argc)) {
            capture_test::PrintUsage(
                argc > 0 ? argv[0] : L"TestDesktopDup.exe",
                L"DXGI Desktop Duplication + window crop (BGR24)");
            return 1;
        }
        ...
        const std::wstring output_path =
            capture_test::ResolveGw2OutputPath(argc, argv, L"Gw2-64-desktopdup.png");

        std::printf("Desktop Duplication capture (Gw2-64.exe), BGR24\n");
        std::printf("  PID:   %lu\n", gw2.process_id);
        std::printf("  HWND:  0x%p\n", gw2.hwnd);
        capture_test::PrintWindowTitle(gw2.hwnd);
        std::printf("  Method: DXGI Desktop Duplication + window crop\n");
        ...
        GC_ImageBgr24 image{};
        const GC_Result capture_result = GC_CaptureWindowDesktopDuplicationBgr24(
            gw2.hwnd,
            GC_FLAG_NONE,
            region_ptr,
            &image);
